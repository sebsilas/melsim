...
benchmark_sim_measure <- function(sim_measure_id) {
  ...
  benchmarks <- tibble::tibble(
              melodic_sim_measure_id = sim_measure_id,
...
sim_measure_benchmarks <-
  purrr::map_dfr(1:max(melodic_sim_measures$melodic_sim_measure_id),
                                         
      function(sim_measure_id) {
        
        if(sim_measure_id %% 100 == 0) {
           logging::loginfo("Benchmarking sim_measure_id: %s", sim_measure_id)
        }
        
        tryCatch({
          benchmark_sim_measure(sim_measure_id)
        }, error = function(err) {
...
# t <- benchmark_sim_measure(1L)

use_data(sim_measure_benchmarks)
...
sim_measure_benchmarks$rmse %>% hist()
  
sim_measure_benchmarks$r %>% hist()

sim_measure_benchmarks$speed_benchmark %>% hist()
...
sim_measure_benchmarks <- sim_measure_benchmarks %>% 
  left_join(melodic_sim_measures, by = "melodic_sim_measure_id")
...
sim_measure_benchmarks_ranked <- sim_measure_benchmarks %>% 
  mutate(rmse_d = d_min(rmse, use_data = TRUE),
         rsq_d = d_max(r, low = 0, high = 1),
         speed_benchmark_d = d_min(speed_benchmark, use_data = TRUE),
         overall = d_overall(across(c(rmse_d, rsq_d))), 
         overall2 = d_overall(across(ends_with("_d")))) %>% 
  arrange(desc(overall))


cor(sim_measure_benchmarks_ranked$overall, 
    sim_measure_benchmarks_ranked$overall2,
...
overall_ecdf <- ecdf(sim_measure_benchmarks_ranked$overall)

sim_measure_benchmarks_ranked <- sim_measure_benchmarks_ranked %>% 
  mutate(overall_percentile = overall_ecdf(overall) )
...
lm1 <- sim_measure_benchmarks_ranked %>% 
  filter(!is.na(overall), !is.nan(overall)) %>% 
  lm(overall ~ 
            # normalisation_method +. only has one level...
            sim_type +
            optimizer +
            low_level_sim_measure_name +
            sim_transformation #+
...
top_five_percentile <- sim_measure_benchmarks_ranked %>% 
  filter(overall_percentile >= .95)
